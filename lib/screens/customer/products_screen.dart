...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final categories = ['All', ...DummyData.categories.where((c) => c['name'] != 'All').map((c) => c['name'].toString())];

    // Filter products based on selected category
    List<Map<String, dynamic>> products = DummyData.freshProducts;
    if (_selectedCategory != 'All') {
      products = products.where((p) {
        return p['category'].toString().toLowerCase() == _selectedCategory.toLowerCase();
      }).toList();
    }
